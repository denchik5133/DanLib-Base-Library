--[[--
Модуль запросов
DanLib предоставляет своим пользователям возможноть взаемодействывать с игроком посредством запросов.
Этими запросами можно получить информацию от игрока, или дать её игроку.

    DanLib.Donate('Дополнительный слот для персонажа', 200,
...
function DanLib.textWrap(text, font, maxWidth)
    local totalWidth = 0
...
-- @usage local material = DanLib.Materials.Information -- материал иконки информации
-- 
-- -- Где-то позже
...
local Materials = DanLib.Materials

if IsValid(DanLibPopupFrame) then DanLibPopupFrame:Remove() end
if IsValid(DanLibOnScreenPopupFrame) then DanLibOnScreenPopupFrame:Remove() end

for k, v in pairs(Materials) do
    local path = 'DanLib/popups/'..string.lower(k)..'.png'
...
local textWrap = DanLib.textWrap
...
hook.Add('DanLib::ConfigLoaded', 'DanLib::Querrys', function()

local Colors = {
    Information = DanLib.Config.Theme.Accent,
    Question = DanLib.Config.Theme.Accent,

    Confirm = DanLib.Config.Theme.Green,
    Warning = DanLib.Config.Theme.Yellow,
    Error = DanLib.Config.Theme.Red,
}

--- Вывод текста на экран, с указаными кнопками
-- @realm client
-- @tparam material|DanLib.Materials mIcon Материал иконки
...
-- @see ButtonStructure
-- @see DanLib.Materials
-- @aliases `DanLib:Querry(mIcon, tIconColor, sText, tButtons)`
function DanLib.Default(mIcon, tIconColor, sText, tButtons)
    if IsValid(DanLibPopupFrame) then DanLibPopupFrame:Remove() end

    DanLibPopupFrame = vgui.Create('DFrame')

    local frame = DanLibPopupFrame
    frame:SetSize(0, 0)
    frame:Center()
    frame:MakePopup() 
    frame:SetTitle('')
    ...
    function frame:AddButtons()
        ...
        for k, v in pairs(tButtons) do
            local button = TDLib('DButton', ButtonBase)
                ...
                :FadeHover()
                :BarHover(DanLib.Config.Theme.Accent)
    ...
    function frame:Paint(w, h)
        surface.SetDrawColor(DanLib.Config.Theme.Background)
        surface.DrawRect(0, 0, w, h)

        surface.SetDrawColor(DanLib.Config.Theme.AccentQ)
...
function DanLib:Querry(mIcon, tIconColor, sText, tButtons)
    return DanLib.Default(mIcon, tIconColor, sText, tButtons)
end
...
---@treturn nil
-- @aliases `DanLib:QuerryDonate(sName, sPrice, fBuy, fCancel)`
function DanLib.Donate(sName, sPrice, fBuy, fCancel)
    DanLib:Querry(Materials.Donate,
        DanLib.Config.Theme.Yellow,
...
function DanLib:QuerryDonate(sName, sPrice, fBuy, fCancel)
    DanLib.Donate(sName, sPrice, fBuy, fCancel)
end
...
-- @treturn nil
-- @aliases `DanLib:QuerryInformation(sText, fOK)`
function DanLib.Information(sText, fOK)
    DanLib:Querry(Materials.Information,
        DanLib.Config.Theme.Yellow,
        sText,
        {
            {
...
function DanLib:QuerryInformation(sText, fOK)
    DanLib.Information(sText, fOK)
end
...
-- @return nil
-- @aliases DanLib:QuerryChoose(sText, fConfirm, fCancel)
function DanLib.Choose(sText, fConfirm, fCancel)
    DanLib:Querry(Materials.Question,
        DanLib.Config.Theme.Yellow,
        sText,
        {
            {
...
function DanLib:QuerryChoose(sText, fConfirm, fCancel)
    DanLib.Choose(sText, fConfirm, fCancel)
end
...
-- @treturn nil
-- @aliases DanLib:QuerryWarning(sText, fConfirm, fCancel)
function DanLib.Warning(sText, fConfirm, fCancel)
    DanLib:Querry(Materials.Warning,
        DanLib.Config.Theme.Yellow,
        sText .. '\nПродолжить?',
        { 
            {
...
function DanLib:QuerryWarning(sText, fConfirm, fCancel)
    DanLib.Warning(sText, fConfirm, fCancel)
end
...
---Вывод поля для ввода
-- @realm client
-- @tparam material|DanLib.Materials mIcon Иконка
-- @tparam color tIconColor Цвет иконки
-- @tparam string sText Текст
...
-- @treturn nil
-- @see DanLib.Materials
-- @aliases `DanLib.QuerryText(mIcon, tIconColor, sText, sDefaultText, sOkText, fCallback, sCancelText, fCancelCallback)`
function DanLib.QuerryTexts(mIcon, tIconColor, sText, sDefaultText, sOkText, fCallback, sCancelText, fCancelCallback)
    if IsValid(DanLibPopupFrame) then DanLibPopupFrame:Remove() end
...
function DanLib:QuerryText(mIcon, tIconColor, sText, sDefaultText, sOkText, fCallback, sCancelText, fCancelCallback)
    DanLib.QuerryTexts(mIcon, tIconColor, sText, sDefaultText, sOkText, fCallback, sCancelText, fCancelCallback)
end

-- 
-- Выбор кнопки
--
function DanLib:QuerryButton(mIcon, tIconColor, sText, nDefaultButton, sOkText, fCallback, sCancelText, fCancelCallback)
    if IsValid(DanLibPopupFrame) then DanLibPopupFrame:Remove() end
...
function DanLib:ScreenNotify(nTime, mIcon, cIconColor, sText, pSender)
    if IsValid(DanLibOnScreenPopupFrame) then DanLibOnScreenPopupFrame:OldRemove() end
    nTime = nTime or 5
    mIcon = mIcon or Materials.Information
    cIconColor = cIconColor or DanLib.Config.Theme.AccentQ

    DanLibOnScreenPopupFrame = vgui.Create('DFrame')

    local frame = DanLibOnScreenPopupFrame 
    frame:SetSize(0, 0)
    frame:SetTitle('')
    frame:ShowCloseButton(false)
    frame:SetDraggable(false)
    ...
    function frame:Paint(w, h)        
        ...
        surface.SetDrawColor(DanLib.Config.Theme.Green)
...
function DanLib:Location(nTime, sText, mBelongsTo)
    local frame = DanLib:ScreenNotify(nTime, nil, nil, sText, pSender)

    function frame:DrawIcon(w, h)
        surface.SetDrawColor(ColorAlpha(DanLib.Config.Theme.AccentQ, self.textalpha or 0))
        surface.SetMaterial(Materials.Locator)

        surface.DrawTexturedRect(10, h * .5 - 32, 64, 64)

        if mBelongsTo ~= nil then
            surface.SetDrawColor(ColorAlpha(DanLib.Config.Theme.Text, self.textalpha or 0))
...
netstream.Hook('DanLib::ScreenNotify', function(sText, sType, nTime)
    sType = sType or 'Information'

    DanLib:ScreenNotify(nTime, Materials[sType], Colors[sType], sText)
end)

end)

concommand.Add('danlib_popup_debug', function()
    DanLib:ScreenNotify(nil, QUERY_MAT_SUCCESS, DanLib.Config.Theme.Green, 'Это всплывающее окно отладки на экране!')
end)
