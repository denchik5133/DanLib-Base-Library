...
surface.CreateFont('DanLibUI.Frame', {
    font = 'Open Sans',
    size = 24,
    weight = 500,
    extended = true
...
function PANEL:Init()
    ...
    self.title:SetFont('DanLibUI.Frame')
    ...
    self.closeBtn.Paint = function(pnl, w, h)
        draw.RoundedBox(0, 0, 0, w, h, ColorAlpha(DanLib.Config.Theme.Red, pnl.Alpha), false, true, false, false)

        surface.SetDrawColor(pnl.CloseButton)
        surface.SetMaterial(DanLib.Config.Materials.CloseButton)
    ...
    self.settingsBtn.Paint = function(pnl, w, h)
        draw.RoundedBox(0, 0, 0, w, h, ColorAlpha(DanLib.Config.Theme.Accent, pnl.Alpha), false, false, false, false)

        surface.SetDrawColor(pnl.CloseButton)
        surface.SetMaterial(DanLib.Config.Materials.SettingsButton)
        surface.DrawTexturedRect(6, 6, w - 12, h - 12)
    end
...
vgui.Register('DanLibUI.Frame', PANEL, 'EditablePanel')