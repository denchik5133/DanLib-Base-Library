...
function PNL:LerpColor(var, to, duration, callback)
	if (!duration) then duration = DanLibUI.TransitionTime end
	...
	anim.Think = function(anim, pnl, fract)
		...
		local newColor = DanLibUI:LerpColor(newFract, anim.StartColor, anim.Color)
		self[var] = newColor
	end
	anim.OnEnd = function()
		if (callback) then
...
function PNL:LerpVector(var, to, duration, callback)
	...
	anim.Think = function(anim, pnl, fract)
		...
		local newColor = DanLibUI:LerpVector(newFract, anim.StartVector, anim.Vector)
		self[var] = newColor
	end
	anim.OnEnd = function()
		if (callback) then
...
function PNL:LerpAngle(var, to, duration, callback)
	...
	anim.Think = function(anim, pnl, fract)
		...
		local newColor = DanLibUI:LerpAngle(newFract, anim.StartAngle, anim.Angle)
		self[var] = newColor
	end
	anim.OnEnd = function()
		if (callback) then
...
function PNL:LerpHeight(height, duration, callback, easeFunc)
	if (!duration) then duration = DanLibUI.TransitionTime end
	if (!easeFunc) then easeFunc = function(a, b, c, d) return DanLibUI:Ease(a, b, c, d) end end
