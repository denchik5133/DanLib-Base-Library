...
function PANEL:Init()
	...
	self:SetContentAlignment( 4 ) -- 4
	self:SetTextInset( 8, 0 )
	self:SetIsMenu( true )
	self:SetSortItems( true )

    self:SetTextColor(c'Text')
    self:SetFont('font_sans_21')
end

function PANEL:Paint(w, h)
    draw.RoundedBox(0, 0, 0, w, h, Color(65, 65, 65, 150)) -- Color(75,75,75)
end
...
function PANEL:OpenMenu( pControlOpener )
	...
	self.Menu = vgui.Create('DanLib.Menu', self)
...
derma.DefineControl( 'DanLibUI.Combobox', '', PANEL, 'DButton' )
...
function PANEL:DrawElements()
	...
	self.comboBox:SetFont( "font_sans_21" )
	for k, v in pairs( self.optionTable ) do
		self.comboBox:AddChoice( DanLib.Localization.LangCodes[k], k )
	end
...
vgui.Register( "DanLibUI.Combobox", PANEL, "DPanel" )]]