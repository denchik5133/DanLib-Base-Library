 ...
 *	 @source 		https://github.com/Jaffies/paint/blob/main/lua/paint/rounded_boxes_cl.lua
 */
...
-- DanLib.RoundedBoxe:Draw( 20, 5, 5, 64, 64, {
...
-- DanLib.RoundedBoxe:Draw( 32, 72, 5, 64, 64, color_white, ( Material( "icon16/application_xp.png" ) ) )
--```
--
--Asymmetrical Example
--Drawing a rounded box with only the top-right and bottom-left corners rounded.
--```lua
--DanLib.RoundedBoxe:roundedBoxEx( 16, 10, 10, 64, 64, color_white, false, true, false, true )
--```
--
...
--```lua
	...
	-- 		mask(function()
	-- 			DanLib.RoundedBoxe:Draw( 32, 0, 0, w, h, COLOR_RED )
	-- 		end,
	-- 		function()
	...
	--
	-- 				DanLib.RoundedBoxe:Draw(radius, rippleX - radius, rippleY - radius, radius * 2, radius * 2, ColorAlpha(color_white, alpha))
	-- 			end
	-- 		end)
	-- 	DanLib.endPanel()
	-- end
...
--
-- DanLib.RoundedBoxe:Draw(32, 10, 10, 300, 128, {color1, color3, color2, color3})
-- -- Center is color3 not nil because interpolating between colors and between HSV is different
--```
---@class DanLib.RoundedBoxe
local roundedBoxes = {}
...
do
	...
	function roundedBoxes:getMeshVertexCount(radius, rightTop, rightBottom, leftBottom, leftTop)
		if (radius > 3) then
			...
			return 10 + (rightTop and vertsPerEdge or 0) + (rightBottom and vertsPerEdge or 0) + (leftBottom and vertsPerEdge or 0) + (leftTop and vertsPerEdge or 0)
		else
			return 10 + (rightTop and 1 or 0) + (rightBottom and 1 or 0) + (leftBottom and 1 or 0) + (leftTop and 1 or 0)
		end
	end
	...
	function roundedBoxes:generateSingleMesh(createVertex, mesh, radius, x, y, endX, endY, leftTop, rightTop, rightBottom, leftBottom, colors, u1, v1, u2, v2, curviness)
		...
		if mesh then
			meshBegin(mesh, PRIMITIVE_POLYGON, self:getMeshVertexCount(radius, rightTop, rightBottom, leftBottom, leftTop))
		end
...
---@diagnostic disable-next-line: deprecated
	local bilinearInterpolation = DanLib.bilinearInterpolation
	...
	---@param curviness number?
	---@private Internal variable. Not meant to use outside
	function roundedBoxes:ExSingle(radius, x, y, w, h, colors, leftTop, rightTop, rightBottom, leftBottom, material, u1, v1, u2, v2, curviness)
		...
		if (meshObj == nil) then
			...
			self:generateSingleMesh(createVertex, meshObj, radius, 0, 0, w, h, leftTop, rightTop, rightBottom, leftBottom, colors, u1, v1, u2, v2, curviness)
...
do
	...
	local prev2 = {}
	local batch = DanLib.batch
	local incrementZ = DanLib.incrementZ
	local color = Color
	local bilinearInterpolation = DanLib.bilinearInterpolation
	...
	---@param curviness number?
	---@private Internal variable. Not meant to use outside
	function roundedBoxes:ExBatched(radius, x, y, w, h, colors, leftTop, rightTop, rightBottom, leftBottom, curviness)
		prev1 = nil
		prev2 = nil
		self:generateSingleMesh(createVertex, nil, radius, x, y, x + w, y + h, leftTop, rightTop, rightBottom, leftBottom, colors, 0, 0, 1, 1, curviness)
	end
end
...
do
	...
	local batch = DanLib.batch
	local getColorTable = DanLib.getColorTable
	...
	function roundedBoxes:Ex(radius, x, y, w, h, colors, leftTop, rightTop, rightBottom, leftBottom, material, u1, v1, u2, v2, curviness)
		if (colors[4] == nil) then colors = getColorTable(4, colors) end
		...
		if batch.batching then
			self:ExBatched(radius, x, y, w, h, colors, leftTop, rightTop, rightBottom, leftBottom, curviness)
		else
			self:ExSingle(radius, x, y, w, h, colors, leftTop, rightTop, rightBottom, leftBottom, material, u1, v1, u2, v2, curviness)
		end
	end
	...
	function roundedBoxes:Draw(radius, x, y, w, h, colors, material, u1, v1, u2, v2, curviness)
		self:Ex(radius, x, y, w, h, colors, true, true, true, true, material, u1, v1, u2, v2, curviness)
	end
end
...
do
	...
	function roundedBoxes:generateDrawPoly(radius, x, y, w, h, leftTop, rightTop, rightBottom, leftBottom, u1, v1, u2, v2, curviness)
		createdTable = {}
		len = 0
		self:generateSingleMesh(createVertex, nil, radius, x, y, w, h, leftTop, rightTop, rightBottom, leftBottom, emptyTab, u1, v1, u2, v2, curviness)
...
do
	...
	function roundedBoxes:generateSimpleRoundedBox(radius, x, y, w, h, colors, leftTop, rightTop, rightBottom, leftBottom)
		...
		meshBegin(iMesh, PRIMITIVE_POLYGON, 10)
			...
			meshColor((color1R + color2R + color3R + color4R) * 0.25, (color1G + color2G + color3G + color4G) * 0.25, (color1B + color2B + color3B + color4B) * 0.25, (color1A + color2A + color3A + color4A) * 0.25)
			...
			meshColor((color1R + color2R) * 0.5, (color1G + color2G) * 0.5, (color1B + color2B) * 0.5, (color1A + color2A) * 0.5)
			meshTexCoord(0, radiusW, 0)
			meshAdvanceVertex()

			meshPosition(x + w, y, 0)
			meshColor(color2R, color2G, color2B, color2A)
			meshTexCoord(0, rightTop and 0 or radiusW, rightTop and 0 or radiusH)
			meshAdvanceVertex()

			meshPosition(x + w, y + halfH, 0)
			meshColor((color3R + color2R) * 0.5, (color3G + color2G) * 0.5, (color3B + color2B) * 0.5, (color3A + color2A) * 0.5)
			meshTexCoord(0, 0, radiusH)
			meshAdvanceVertex()

			meshPosition(x + w, y + w, 0)
			meshColor(color3R, color3G, color3B, color3A)
			meshTexCoord(0, rightBottom and 0 or radiusW, rightBottom and 0 or radiusH)
			meshAdvanceVertex()

			meshPosition(x + halfW, y + w, 0)
			meshColor((color3R + color4R) * 0.5, (color3G + color4G) * 0.5, (color3B + color4B) * 0.5, (color3A + color4A) * 0.5)
			meshTexCoord(0, radiusW, 0)
			meshAdvanceVertex()

			meshPosition(x, y + w, 0)
			meshColor(color4R, color4G, color4B, color4A)
			meshTexCoord(0, leftBottom and 0 or radiusW, leftBottom and 0 or radiusH)
			meshAdvanceVertex()

			meshPosition(x, y + halfW, 0)
			meshColor((color1R + color4R) * 0.5, (color1G + color4G) * 0.5, (color1B + color4B) * 0.5, (color1A + color4A) * 0.5)
			meshTexCoord(0, 0, radiusH)
			meshAdvanceVertex()
	...
	function roundedBoxes:drawSimpleRoundedBoxEx(radius, x, y, w, h, colors, leftTop, rightTop, rightBottom, leftBottom)
		...
		if (meshObj == nil) then
			meshObj = self:generateSimpleRoundedBox(radius, 0, 0, w, h, colors, leftTop, rightTop, rightBottom, leftBottom)
	...
	function roundedBoxes:drawSimpleRoundedBox(radius, x, y, w, h, colors)
		self:drawSimpleRoundedBoxEx(radius, x, y ,w, h, colors, true, true, true, true)
	end
...
DanLib.RoundedBoxe = roundedBoxes